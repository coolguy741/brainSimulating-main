...
BabylonScene.prototype = {
    ...
    setdefaultPP: function (state) {
        if (state === true) {
           ...
           // this.defaultPLR.imageProcessing.vignetteColor = BABYLON.Color3.FromHexString("#13EBF7").toLinearSpace();
            this.defaultPLR.imageProcessing.vignetteColor = BABYLON.Color3.FromHexString("#b7adb6").toLinearSpace();
    ...
    postLoad: function () {
        ...
        for (var prop in this.application.brainSectionData) {
            ...
            if (section.material) {
                section.material.albedoColor = BABYLON.Color3.FromHexString(section.highlightColor).toLinearSpace();
        ...
        this.brainMaterial.albedoColor = BABYLON.Color3.FromHexString("#ffffff").toLinearSpace();
        this.brainMaterial.emissiveColor = BABYLON.Color3.FromHexString("#1B2729").toLinearSpace();
        this.brainMaterial.subSurface.isRefractionEnabled = false;
        this.brainMaterial.subSurface.tintColor = BABYLON.Color3.FromHexString("#ff00dc").toLinearSpace();
        this.brainMaterial.subSurface.tintColorAtDistance = 0;
        this.brainMaterial.subSurface.volumeIndexOfRefraction = 3;


        var tl = gsap.timeline({ onComplete: this.onAnimation1Complete.bind(this) });
        tl.from(this.brain.scaling, { x: 0, y: 0, z: 0, duration: 0.45, ease: Back.easeOut, delay: 2 })
        tl.from(this.brain.rotation, { y: 12.56, duration: .35, ease: Power4.easeIn })
        ...
        for (let i = 0; i < a.length; i++) {
            ...
            tl.to(item.position, { x: ob.position2.x, y: ob.position2.y, z: ob.position2.z, duration: 0.5, ease: Power1.easeOut }, "someLabel");
            // tl.to(item.scaling, { x: 0.3, y: 0.3, z: 0.3, duration: 1, ease: Power4.easeOut }, "someLabel");
            tl.to(item.position, { x: ob.position1.x, y: ob.position1.y, z: ob.position1.z, duration: 0.2, ease: Back.easeIn, delay: 0.2 }, "sl");
    ...
    onAnimation1Complete() {
        ...
        this.brainMaterial.clearCoat.roughness  = 0.5;
        ...
        this.lightmapBrainTex = new BABYLON.Texture('/brain3dapplication/assets/images/brain_diffuselighting_invert_sparkle_x1024.jpg');         
        this.brainMaterial.lightmapTexture =  this.lightmapBrainTex;
        this.brainMaterial.lightmapTexture.level = 0.47;
        this.brainMaterial.lightmapTexture.vScale = -1;



        this.t2 = gsap.to(this.brain.scaling, { x: 1.005, y: 1.005, z: 1.005, duration: 5, ease: Linear, repeat: -1, yoyo: true })
        ...
        for (var prop in this.application.brainSectionData) {
            ...
            if (item.handleNodeVisibilityWithEffect) {
                ...
                glowTween.to(item.material, { emissiveIntensity: item.emmissiveScalar*0.1, alpha: 1, duration: 0.1, ease: Linear }, "-=.05")
            } else {
                glowTween.to(item.material, { emissiveIntensity: item.emmissiveScalar*0.1, duration: 0.1, ease: Linear }, "-=.05")
            }
        }
    ...
    create: function (canvas) {
        ...
        this.camera.targetScreenOffset.x = -0.9;
        this.camera.targetScreenOffset.y = 0.2;
        ...
        this.changeBackgroundColor("#666666");
